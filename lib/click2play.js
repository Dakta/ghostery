...
define([
	...
	function Click2playDb() {
		...
		this.reset = function (tab_url) {
			if (!allowOnceList.hasOwnProperty(tab_url)) {
				return;
			}

			var list = allowOnceList[tab_url];
			...
			if (!_.size(list)) {
				delete allowOnceList[tab_url];
			}
		};

		this.allowedOnce = function (tab_url, aid) {
			return allowOnceList.hasOwnProperty(tab_url) &&
				allowOnceList[tab_url].hasOwnProperty(aid);
		};

		this.allowOnce = function (app_ids, tab_url) {
			if (!allowOnceList.hasOwnProperty(tab_url)) {
				allowOnceList[tab_url] = {};
			}

			// the page will be reloaded, but we want to keep allowing
			// everything else that's been "allowed once"
			for (var aid in allowOnceList[tab_url]) {
				if (allowOnceList[tab_url].hasOwnProperty(aid)) {
					allowOnceList[tab_url][aid]++;
				}
			}
			...
			app_ids.forEach(function (app_id) {
				allowOnceList[tab_url][app_id] = 2;
			});
		};
